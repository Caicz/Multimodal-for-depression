...
def train():
    ...
    if args.use_vlm:
        file_name = f"user_sequences_{args.dataset}_with_vlm_train.jsonl"
        file_test_name = f"user_sequences_{args.dataset}_with_vlm_test.jsonl"
        data_test_path = data_dir / file_test_name 
    ...
    if args.use_vlm:
        train_data = all_data
        test_data, val_data = train_test_split(test_all_data, test_size=0.3333, random_state=42)
    else:    
    ...
    try:
        ...
        vlm_tokenizer = None
        if args.use_vlm:
            # 始终为 VLM 加载中文 Tokenizer，以匹配 HierarchicalMultimodalModel 中的 vlm_bert (hfl/chinese-roberta-wwm-ext)
            vlm_tokenizer = AutoTokenizer.from_pretrained('hfl/chinese-roberta-wwm-ext')
            print("已加载额外中文 Tokenizer 用于 VLM 总结处理")
    ...
    for epoch in range(args.epochs):
        ...
        for i, batch in enumerate(train_loader):
            ...
            with torch.amp.autocast(device_type=device.type, dtype=torch.float32):
                outputs = model(ids, mask, pixels, s_mask, densities, rts, 
                                summary_ids=sum_ids if args.use_vlm else None, 
                                summary_mask=sum_mask if args.use_vlm else None)
        ...
        if acc > best_acc:
            ...
            torch.save(model.state_dict(), f"../models/best_model_{args.dataset}.pt")
            evaluate_on_test(model, test_data, tokenizer, device, f"../results/report_{args.dataset}.txt", use_vlm=args.use_vlm, vlm_tokenizer=vlm_tokenizer)
        else:
            no_improve += 1
