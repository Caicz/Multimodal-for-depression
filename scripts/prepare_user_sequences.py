...
def build_sequences(data_root, high_risk_file, output_file, max_len=25, hr_ratio=0.8):
...
def main():
    ...
    parser.add_argument('--dataset', type=str, default='twitter', choices=['swdd', 'twitter', 'all'],
                        help='指定处理的数据集: swdd, twitter 或 all (默认)')
    ...
    for task in tasks:
        build_sequences(task['root'], task['in'], task['out'])
